"""
PR Creativity Analysis - Uses same structure as ads and social media
"""
import os
import re
import json
...
def run_creativity_analysis(df: pd.DataFrame, media_type: str, 
    ...
    with pd.ExcelWriter(out_xlsx, engine="xlsxwriter") as writer:
        ...
        used_sheet_names = {"Overall Ranking"}
        
        for brand, g in topk_df.groupby(brand_column):
            base_sheet = (str(brand)[:31]) if str(brand).strip() else "(blank)"
            sheet = base_sheet
            
            # Make sheet name unique if it's already used
            counter = 1
            while sheet.lower() in used_sheet_names:
                sheet = f"{base_sheet}_{counter}"
                counter += 1
...
def analyze_creativity_for_month(year: int, month: int, output_folder: str) -> str:
    """
    Run creativity analysis for PR data for a specific month
    Returns path to output file
    """
    from utils.data_processor import load_new_data
    
    # Load and filter PR data for the month
    df = load_new_data("pr", year, month)
    if len(df) == 0:
        raise ValueError(f"No PR data found for {year}-{month:02d}")
    
    # Get column names from config
    media_config = MEDIA_TYPES["pr"]
    text_column = media_config["text_column"]
    brand_column = media_config["brand_column"]
    reach_column = media_config["reach_column"]
    
    # Run analysis
    output_file = run_creativity_analysis(df, "pr", text_column, brand_column, reach_column)
    
    # Move to output folder (overwrite if exists)
    os.makedirs(output_folder, exist_ok=True)
    final_output = os.path.join(output_folder, f"creativity_analysis_pr.xlsx")
