"""
Creativity Analysis - Merged for Ads, PR, and Social Media
"""
import os
import re
import json
...
def rank_brands_cross_brand(payload: List[Dict[str, Any]], media_type: str) -> Dict[str, Any]:
    ...
    user_input = (header + context +
        ...
        "- examples (array of strings; each max 50 characters, properly escaped)\n"
...
def run_creativity_analysis(df: pd.DataFrame, media_type: str, 
    ...
    with pd.ExcelWriter(out_xlsx, engine="xlsxwriter") as writer:
        ...
        for brand, g in topk_df.groupby(brand_column):
            ...
            g.sort_values(reach_column, ascending=False)[[brand_column, reach_column, text_column]].to_excel(writer, sheet_name=sheet, index=False)
        ...
        if skipped_brands:
            ...
            skipped_df.to_excel(writer, sheet_name="Skipped Brands", index=False)
        
        # Debug sheets
        pd.DataFrame(selections).to_excel(writer, sheet_name="Selections JSON", index=False)
        top_per_brand.to_excel(writer, sheet_name="TopN Considered", index=False)
...
def analyze_creativity_for_month(year: int, month: int, output_folder: str) -> str:
    """
    Run creativity analysis for ads data for a specific month
    ...
    media_config = MEDIA_TYPES["ads"]
    text_column = media_config["text_column"]
    brand_column = media_config["brand_column"]
    reach_column = media_config["reach_column"]
    
    # Run analysis
    output_file = run_creativity_analysis(df, "ads", text_column, brand_column, reach_column)
    
    # Move to output folder (overwrite if exists)
    os.makedirs(output_folder, exist_ok=True)
    final_output = os.path.join(output_folder, f"creativity_analysis_ads.xlsx")
