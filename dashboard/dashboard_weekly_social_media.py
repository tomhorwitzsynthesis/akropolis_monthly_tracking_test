...
def load_data():
    ...
    # Split the analysis period into two periods (first 7 days, then remaining days)
    from datetime import timedelta
    
    prev_7_days_start = last_14_days_start
    prev_7_days_end = last_14_days_start + timedelta(days=6)  # First 7 days
    current_7_days_start = prev_7_days_end + timedelta(days=1)
...
# ---- UI controls ----
st.title(f"Facebook Social Media Intelligence – {config.ANALYSIS_START_DATE.strftime('%B %d')}-{config.ANALYSIS_END_DATE.strftime('%d, %Y')} Analysis")
            ...
            for i, brand in enumerate(tab_names):
                with summary_tabs[i]:
                    ...
                    <div style="border: 2px solid {border_color}; padding: 20px; border-radius: 10px; margin: 10px 0; background-color: {bg_color};">
                        ...
                        <p style="margin: 0; line-height: 1.6; color: #333; font-size: 14px;">{relevant_summaries[brand]}</p>
                    </div>
                    """, unsafe_allow_html=True)
        else:
